 ...
 * submits it to the configured x402 facilitator for Base Sepolia settlement.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result) {
      return NextResponse.json(
        { error: 'Payment settlement failed. Please check your Base Sepolia USDC balance and try again.' },
        { status: 500 }
      )
    }
